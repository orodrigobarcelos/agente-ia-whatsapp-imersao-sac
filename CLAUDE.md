...
- **Vá DIRETO na URL de busca** (`/busca?q=...`) em vez da home/autocomplete. Dois motivos: (1) `page.fill`/`page.type` quase nunca disparam o dropdown React sob automação; (2) a home costuma disparar **captcha** ("confirme que é humano") que a URL de busca não dispara. Exemplo real do Reclame Aqui: usar `https://www.reclameaqui.com.br/busca/?q=<empresa>` funciona, enquanto `https://www.reclameaqui.com.br/home/E/` cai no captcha. Procure SEMPRE o padrão `/busca` ou `/search` do site antes de simular digitação.
