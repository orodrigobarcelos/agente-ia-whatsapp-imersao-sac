...
- **Vá DIRETO na URL de busca** (`/busca?q=...`) em vez da home/autocomplete — `page.fill`/`page.type` quase nunca disparam o dropdown React sob automação, e navegar direto é mais determinístico. Exemplo real: no Reclame Aqui use `https://www.reclameaqui.com.br/busca/?q=<empresa>`. Procure SEMPRE o padrão `/busca` ou `/search` do site antes de simular digitação. (Isso resolve o **autocomplete** — NÃO confunda com o captcha, que é problema separado, ver abaixo.)
- **Captcha "Confirme que é humano" = Cloudflare Turnstile.** Aparece em QUALQUER página (home, `/empresa/<slug>`, `/busca`) quando o Cloudflare detecta automação — não é causado pela escolha de URL. Quem resolve é o **stealth** (`chromium.use(StealthPlugin())`, já no template). O título de aba "Um momento…" é a splash do Cloudflare; trate com `waitForFunction` + reload (ver abaixo). Se mesmo com stealth o captcha persistir, o site endureceu e precisaria de solução paga (captcha solver) ou outra fonte de dados.
